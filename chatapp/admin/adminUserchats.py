...
receiver_id = form.getvalue("receiver_id")  # Selected User (Receiver)

# Fetch all verified users
cur.execute("SELECT id, firstname FROM userreg WHERE block_status='' AND otp_status='verified'")
users = cur.fetchall()

# Fetch messages if both sender and receiver are selected
messages = []
...
<body>
    ...
    <div class="sidebar" id="sidebar">
       ...
       <a href="adminUsers.py" class="fw-bold"> Manage Users</a>
        ...
        <a href="adminUserchats.py" class="fw-bold active1">  User Chats </a>
...
<div class="container">
    <h2 class="text-center">Admin Chat Logs</h2>
    <div class="chat-container">
        <div class="chat-box">
        <h5 style="overflow-y:hidden;"> select sender, receiver for view their chat:</h5>
            <form method="POST">
                <div class="mb-3">
                
                    <label for="senderSelect" class="form-label">Sender:</label>
                    <select name="sender_id" id="senderSelect" class="form-control" required onchange="updateReceiverOptions()">
...
print("""
                ...
                <div class="mb-3">
                    <label for="receiverSelect" class="form-label">Receiver:</label>
                    <select name="receiver_id" id="receiverSelect" class="form-control" required>
                        <option value="">Select Receiver</option>
""")

for user in users:
    if str(user[0]) != str(sender_id):  # Exclude sender from receiver list
        selected = "selected" if str(user[0]) == str(receiver_id) else ""
        print(f'<option value="{user[0]}" {selected}>{user[1]}</option>')
...
print("""
            ...
            <div class="mt-4  ">
                <h4 style="overflow-y:hidden;">Conversation: </h4>
                <div class="chat-body">
""")

if not messages:
    print("<p class='text-muted'>Select a sender and receiver to view messages.</p>")
else:
    for message in messages:
        ...
        <div class='{sender_class} chat-message'>
            ...
            <small class="text-muted">{message[3]}</small>
        </div>
        """)
